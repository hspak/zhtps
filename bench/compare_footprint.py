"""Compare prebuilt layout variants with pinned workers and user-space PMU counters."""
...
EVENTS = ('cycles:u', 'instructions:u', 'cache-misses:u', 'L1-dcache-load-misses:u')
CASES = {
    'small': dict(connections=64, capacity=256, server_cpus=[2], client_cpus=[4, 5, 6, 7]),
    'many': dict(connections=4096, capacity=8168, server_cpus=[2], client_cpus=[4, 5, 6, 7]),
    'multicore': dict(connections=16384, capacity=4096, server_cpus=list(range(8)), client_cpus=list(range(8, 16))),
    'latency': dict(connections=1, capacity=256, server_cpus=[2], client_cpus=[4, 5, 6, 7]),
}
    ...
    with prefix.with_suffix('.server.log').open('w') as log:
        try:
            ...
            load_command = ['taskset', '-c', ','.join(map(str, case['client_cpus'])), str(client),
                            ...
                            '-warmup', f'{options.warmup}s', '-duration', f'{options.duration}s']
            ...
            assert load['attempts'] == load['successes'] and load['successes'] > 0
            for key in ('connections_ready', 'connections_measured', 'connections_opened'):
                assert load[key] == case['connections'], (key, load[key])
...
def main():
    ...
    parser.add_argument('--workspace', type=Path, default=Path('/tmp/zhtps-footprint'))
    parser.add_argument('--records', type=Path, default=ROOT / 'zig-out/bench/request-footprint')
    parser.add_argument('--output', type=Path, required=True)
    parser.add_argument('--cases', nargs='+', choices=CASES, default=['small', 'many', 'multicore'])
    ...
    client = ROOT / 'zig-out/bench/load'
    ...
    report = dict(timestamp_utc=time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
                  ...
                  pmu_scope='user-space server threads, including client setup/warmup/drain', runs=[])
        ...
        for repeat in range(args.repeats):
            ...
            for name in order:
                ...
                brief = dict(case=case_name, variant=name, repeat=repeat+1,
                             goodput=row['load']['window_successes_per_second'],
                             p99_us=row['load']['latency_us']['p99'], cpu_ns=row['cpu_ns_per_completed'],
