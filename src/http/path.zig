...
const log = std.log.scoped(.http_path);
...
pub fn normalize(buffer: []u8, path: []const u8) Error![]const u8 {
    ...
    if (path.len > 0 and path[0] != '/' and !std.mem.eql(
        u8,
        path,
        "*",
    ...
    while (read < path.len) {
        ...
        if (std.ascii.isAlphanumeric(byte) or std.mem.indexOfScalar(
            u8,
            "-._~",
            byte,
        ) != null) {
    ...
    while (read < len) {
        ...
        if (std.mem.startsWith(
            ...
            written = std.mem.lastIndexOfScalar(
                u8,
                buffer[0..written],
                '/',
            ) orelse 0;
