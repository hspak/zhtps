//! A completion-driven HTTP origin server. Application hooks run on the owning worker.
...
pub const RunError = std.mem.Allocator.Error || platform.Error || Config.Error || std.Thread.SpawnError || error{
    IoUringUnavailable,
...
pub fn Server(comptime App: type) type {
    return struct {
        ...
        /// Runs one event-loop thread and private ring per configured worker.
        /// Connection and admission budgets apply per worker; admin slots are
        /// reserved once on worker zero. The caller owns stop and the allocator,
        /// which is used only on the calling thread. All workers join and all
        /// in-flight operations finish before storage is freed, including errors.
        pub fn run(gpa: std.mem.Allocator, config: Config, stop: *const std.atomic.Value(bool)) RunError!void {
            var resolved = try config.resolve();
            const workers = try gpa.alloc(Self, config.workers);
            defer gpa.free(workers);
            var shared: Shared = .{ .workers = workers };
            var initialized: usize = 0;
            defer for (workers[0..initialized]) |*worker| worker.deinit();
            for (workers, 0..) |*worker, id| {
                try worker.init(gpa, resolved, stop, @intCast(id), &shared);
                initialized += 1;
                if (id == 0) {
                    resolved.port = worker.listener.port;
                    resolved.admin_port = worker.admin_listener.port;
                    worker.config = resolved;
                }
            }
            const threads = try gpa.alloc(std.Thread, config.workers - 1);
            defer gpa.free(threads);
            var started: usize = 0;
            // This also joins workers when a later thread cannot be created.
            defer {
                shared.abort.store(true, .monotonic);
                for (threads[0..started]) |thread| thread.join();
            }
            for (threads, workers[1..]) |*thread, *worker| {
                thread.* = try std.Thread.spawn(.{}, workerMain, .{worker});
                started += 1;
            }
            workers[0].logger.emit(.{
                .timestamp_ns = platform.realtimeNs(),
                .event = "listening",
            ...
            workers[0].logger.emit(.{
                ...
                .address = resolved.admin_address,
        ...
        fn init(self: *Self, gpa: std.mem.Allocator, config: Config, stop: *const std.atomic.Value(bool), worker_id: u32, shared: *Shared) RunError!void {
            ...
            const listener = try platform.listen(config.address, config.port, 128);
            errdefer if (!transferred) platform.close(listener.fd);
            const admin_listener = if (worker_id == 0)
                try platform.listen(config.admin_address, config.admin_port, 32)
            else
            ...
            const per_connection = config.header_bytes + config.trailer_bytes + config.receive_bytes +
            ...
            const log_slots = try gpa.alloc(Logger.Slot, config.log_slots);
            ...
            self.* = .{
                ...
                .worker_id = worker_id,
                .public_free = 0,
                .admin_free = if (worker_id == 0) config.max_connections else null,
                .gpa = gpa,
                .ring = ring,
                .listener = listener,
            ...
            for (connections, 0..) |*connection, index| {
                ...
                connection.parser.init(head, trailers, .{ .max_body_bytes = config.max_body_bytes });
        ...
        fn queueLog(self: *Self) RunError!void {
            ...
            // cannot extend its stderr ownership indefinitely as records arrive.
            ...
            const sqe = self.ring.writev(control(.log_write), 2, self.log_iovecs[0..self.log_batch_remaining], std.math.maxInt(u64)) catch
                return error.IoUringResources;
        ...
        fn respondStatus(self: *Self, index: usize, status: u16, close: bool) RunError!void {
            ...
            const fields = [_]http.Header{.{ .name = "Allow", .value = allow }};
            const headers: []const http.Header = if (status == 405) &fields else &.{};
        ...
        fn receiveAdminHead(self: *Self, index: usize) RunError!bool {
            ...
            const precondition = http.conditions.evaluate(request, .{}, platform.realtimeNs() / 1_000_000_000) catch {
                try self.respondStatus(index, 400, true);
                return true;
            };
            if (precondition) |status| {
                try self.startResponse(index, .{
                    ...
                    .body = if (status == 304) .{ .stream = null } else .{ .bytes = "" },
        ...
        fn startResponse(self: *Self, index: usize, initial: http.Response) RunError!void {
            ...
            const second = platform.realtimeNs() / 1_000_000_000;
        ...
        fn beginShutdown(self: *Self, now: u64) RunError!void {
            ...
            self.logger.emit(.{ .timestamp_ns = platform.realtimeNs(), .event = "shutdown_started" });
...
test "fatal completion drains existing receives before returning" {
    ...
    const TestServer = Server(application);
...
test "fatal worker error stops and joins the other event loops" {
    ...
    workers[0].init(testing.allocator, config, &stop, 0, &shared) catch |err| switch (err) {
    ...
    try workers[1].init(testing.allocator, config, &stop, 1, &shared);
...
test "unused slots do not invoke application initialization on the setup thread" {
    ...
    const TestServer = Server(App);
    var workers: [1]TestServer = undefined;
    var shared: TestServer.Shared = .{ .workers = &workers };
    var stop: std.atomic.Value(bool) = .init(false);
    App.initializations = 0;
    workers[0].init(testing.allocator, .{ .max_connections = 2, .port = 0, .admin_port = 0 }, &stop, 0, &shared) catch |err| switch (err) {
        error.IoUringUnavailable => return error.SkipZigTest,
        else => return err,
    };
    defer workers[0].deinit();
...
test "embedded worker applies automatic admission before filling its connections" {
    ...
    var shared: TestServer.Shared = .{ .workers = &workers };
    var stop: std.atomic.Value(bool) = .init(false);
    workers[0].init(testing.allocator, .{
        .max_connections = 4,
        .port = 0,
        .admin_port = 0,
...
test "stream batching preserves producer capacity and a deferred fragment" {
    ...
    const TestServer = Server(TestApp);
